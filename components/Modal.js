...
const Modal = () => {
    ...
    const deleteComment = (item) => {
        deleteData(`comment/${item.id}`, auth.token)
        .then(res => {
            if(res.err) return dispatch({type: 'NOTIFY', payload: {error: res.err}})
    ...
    const deleteOrder = (item) => {
        deleteData(`order/${item.id}`, auth.token)
        .then(res => {
            if(res.err) return dispatch({type: 'NOTIFY', payload: {error: res.err}})
    ...
    const handleSubmit = () => {
        if(modal.length !== 0){
            for(const item of modal){
                ...
                if(item.type === 'ADD_COMMENT') deleteComment(item)

                if(item.type === 'ADD_ORDERS') deleteOrder(item)
